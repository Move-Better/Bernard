...
|---|---|---|---|---|---|
| **F1** | **The Weekly Call** — natural realtime-voice conversation as the default & only required input (inbound tap or outbound call Bernard places) | 🎙️ Voice UX | 🚀 | Open | Gemini Live (free transcripts) / OpenAI Realtime / ElevenLabs Conversational AI. Promote existing `PhoneCall.jsx` + `/api/realtime-session` (gated by `realtime_voice_enabled`) to default. |
...
| **F3** | ~~Generative video studio — auto-b-roll + avatar~~ → **REDEFINED: the Moment-Miner** — mine real MIKED clinical footage for gold moments → Reel with real audio + visual + captions; clone voice = gap-fill only | 🛰️ Frontier | 🚀 | Closed→Redefined | **Generative video REJECTED 2026-06-21** (prototyped Veo 3.1 + Runway Gen-4.5 + ElevenLabs clone on real Move Better content; Q: "AI slop… breaks the whole moat"). Synthetic FACE/avatar also ruled out. Survivor: cloned VOICE (on-moat) + assembling the clinician's OWN miked footage. Foundation gap: 1/373 clips transcribed. See `memory/project-moment-miner.md` + `memory/feedback-no-ai-generated-video.md`. |
