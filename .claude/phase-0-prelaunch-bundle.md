...
**Generated 2026-05-21.** Companion to `project_bernard_revenue_roadmap.md` Phase 0 checklist.
...
|---|---|
| Name | Bernard |
| LinkedIn URL | `bernard` (or `bernard-ai` if taken) |
| Website | `https://withbernard.ai` |
...
| Logo | `public/brand/bernard-icon-512.png` |
| Cover image | `public/brand/bernard-linkedin-cover-1128x191.png` |

**About section (paste verbatim, 3 paragraphs):**

> Bernard is the content engine built for hands-on, integrative providers — chiropractors, PTs, OTs, naturopaths, massage therapists, acupuncturists — the practices that share patients and referrals but get ignored by tools built for surgeons and hospital systems.
>
> The premise is simple: you already know what to say. You just don't have time to write it down. Bernard interviews you for 12 minutes, then drafts the blog post, the newsletter, the LinkedIn caption, and the social pieces in your actual voice — phrases, cadence, opinions, and all. You approve. It publishes. You stay in clinic.
>
> Built by a chiropractor for clinics like his own. Move Better — three clinics across people, equine, and animals — runs on Bernard. So does the company itself. If it can't carry our own voice, it has no business carrying yours.

**Admins:** add Michael (`drq@movebetter.co`).

**First content to pin:** the founder story already live at `withbernard.ai/blog/the-four-year-silence-why-clinicians-don-t-write-and-why-that-s-not-their-fault` — cross-post manually as a Page article and pin to the top of the Page feed.

---

## 2. Site analytics on withbernard.ai (~30 min)
...
4. Hit `https://withbernard.ai/blog` from a private/incognito window with no ad blocker
5. Wait 60–90s, refresh the Analytics tab, confirm the page view registered
6. Bookmark the dashboard URL as `Vercel Analytics — Bernard` for the Sunday operating cadence

**After enabling, paste this into `project_bernard_revenue_roadmap.md` Phase 0 row:**
- `✅ Vercel Analytics enabled YYYY-MM-DD — https://vercel.com/movebetter/bernard/analytics`

**If you want to switch to Plausible later:** sign up at plausible.io, add `withbernard.ai`, drop the script tag in the marketing site `<head>`, set custom goals `Newsletter Subscribe` and `Demo Booked`. ~$9/mo. Worth it only if Vercel Analytics' aggregation isn't deep enough by end of Phase 1.
...
- Workspace name: Bernard

**Create a new Collection form. Paste these settings:**

| Field | Value |
|---|---|
| Form title | Bernard clinician testimonials |
| Form subtitle | Share what changed for you and your practice. We may feature your words on withbernard.ai or LinkedIn — only with your permission below. |
...
3. **What did Bernard change for you?** — long text, required. Placeholder: `Before Bernard, our blog hadn't been updated in… (be specific — time saved, pieces published, patient/client response)`
...
5. **Permission to publish** — checkbox, required. Label: `Yes — Bernard may quote me by name on withbernard.ai, LinkedIn, and marketing emails. (I can withdraw permission anytime by emailing drq@withbernard.ai.)`
...
**After creation:** grab the public collection URL (looks like `https://senja.io/p/bernard/...`) and paste into `project_bernard_revenue_roadmap.md` Phase 0 row:
...
- Base: `https://calendly.com/<your-handle>/bernard-demo`
...
**Folder for assets:** create `public/marketing/ads/` in the repo (or a `Bernard / Marketing / Ads` Drive folder — pick whichever you'll actually open, then commit the path here).
...
> "I'm a chiropractor. I built Bernard because the writing kept not happening."
...
>
> I built Bernard so the writing stops losing. 12-minute interview. Voice-true drafts in your actual cadence. You approve, it publishes.
>
...
>
> No copywriter. No content team. One short interview per cycle, drafted by Bernard in the clinician's actual voice, reviewed by them, published.
>
...
>
> Bernard changes the job. Talk for 12 minutes. We draft. You approve. The content gets out the door without anyone pretending to be a writer.
...
**Visual:** 15-second Loom screen recording. Show: (1) hit Start Interview, (2) 2-3 seconds of question + speaking, (3) cut to draft appearing, (4) cut to publish button + live post on withbernard.ai/blog. End frame: Bernard logo + "12 minutes. From your voice."
...
>
> Bernard interviews you, drafts in your real cadence, and publishes when you approve. No copywriter, no template feel.
...
- What's the moment in Bernard's pipeline where voice is most likely to be lost, and what protects it there?
- If a competitor copied every Bernard feature except the voice work, what would their output sound like?
...
- What lines should never be crossed regardless of voice — and how does Bernard enforce them?
...
**Interview seed:**
- Walk me through everything a single 12-minute interview becomes in Bernard's pipeline.
- What's the hourly-rate math for a clinic owner who switches from "I'll write it this weekend" to Bernard?
...
- Where does this audience end — who's NOT a Bernard customer?
...
### Hook (1 paragraph)
The exact moment Move Better's content went silent and how long it stayed silent before Bernard.
...
| Metric | Before Bernard | After (4 weeks) |
|---|---|---|
...
**Pull from:**
- Move Better Astro / WordPress publish history (count posts shipped since first Bernard publish)
...
- "Bernard didn't make us writers. It made writing stop blocking us."
...
And drop this row into `project_bernard_revenue_roadmap.md` Phase 0 checklist:
- `✅ Move Better case study published YYYY-MM-DD — withbernard.ai/blog/<slug> — pull-quote: "<...>"`

---

## End-of-chip runbook checklist

Paste into `project_bernard_revenue_roadmap.md` Phase 0 row when complete:

- [ ] LinkedIn Company Page live, founder story pinned — `linkedin.com/company/bernard`
- [ ] Vercel Analytics firing, dashboard bookmarked — `https://vercel.com/movebetter/bernard/analytics`
