...
**E2E smoke red on every merge with `strict mode violation: ... resolved to 2 elements` = a loose locator that a newly-added sibling now also matches.** Distinct from the two failure modes below (this is one *spec* failing, not setup — the email is "1 failed", not "All jobs have failed"). A regex/partial locator like `getByRole('heading', { name: /integrations/i })` silently depends on there being exactly one matching node on the page. When a later PR adds a second match on a covered route (2026-06-21: #1494 added an `<h2>Other integrations</h2>` to `/settings/integrations`, colliding with the `<h1>Integrations</h1>` the spec waited for), Playwright strict mode fails. Fix: make the locator exact (`{ name: 'Integrations', exact: true }`) or pin the level (`{ level: 1 }`). **Prevention — prefer exact/level-pinned locators in specs from the start; a loose `/regex/i` heading match is a latent strict-mode break waiting for any sibling heading to share a word.** Catch earlier: (1) run `npm run e2e` (or the single spec) against the Vercel **preview** URL before merging any PR that touches a covered route's markup — the post-deploy smoke is too late by design; (2) the Definition-of-Done grep already in this file (`grep -rn "<label>" tests/e2e/`) catches *renames* but NOT *additions* of a word an existing loose locator matches — so when adding a heading/label to a covered page, also grep the specs for any **substring** of the new text (`grep -rin "integrations" tests/e2e/`) and tighten any loose locator it surfaces. Fixed in #1496.
