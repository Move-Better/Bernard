...
**A mockup that HAND-DRAWS a conditional state signs Q off on a picture the code will never produce — derive every state-driven element in the mock from the rule you're going to implement, applied to the real data.** A mockup is normally a layout question, but the moment it contains an element whose appearance depends on a *rule* (a severity/alert colour, a badge that appears past a threshold, an empty state, a disabled control), hand-authoring how it looks is no longer illustration — it's an unverified claim about what the rule does. The mock looks right, Q approves it, and the shipped code disagrees, because the rule was never run against real numbers. (2026-07-22, the /week schedule strip #2257: the mock stated the rule as "under target with days left" and drew exactly ONE amber chip — Facebook 0/3 — leaving Google Business 1/2 and LinkedIn 2/3 neutral because that read well. The implemented rule was the stated one, `!isPast && got < target`, which on the same real week lights **three of five** chips. Q approved a strip with one amber chip and got one with three; it took a follow-up PR (#2260) to make the amber pace-aware so reality matched the picture.) **Rules: (1) when a mock contains a conditional state, write the rule down explicitly next to it AND apply it by hand to every row of the real data you put in the mock — if the rule says three chips go amber, draw three, even when it looks bad; the mock looking bad IS the finding, and it surfaces before the PR instead of after. (2) A rule that produces an ugly mock is usually a bad rule, not a bad mock — that's the cheapest possible moment to discover it. (3) For any threshold/severity rule, tabulate the outcome across the full range of the dimension it varies over before shipping (here: a 5-line node script printing all 5 channels × all 7 weekdays), and paste that table into the PR — it took ~30 seconds and would have caught this pre-merge.**
...
**Label third-party concepts with the platform's OWN vocabulary — verified from the code that calls it, not invented because a compound reads clearer.** When a surface exposes an external platform's entities (post formats, statuses, channel types, account tiers), the temptation is to coin a label that seems more explanatory to the user. That label is an unverified claim about the platform's data model, and a wrong one teaches staff a taxonomy the platform doesn't have — then breaks when they cross-reference the real app. (2026-07-22, the T3 mockup: I labeled Instagram's format mix "Post/Carousel · Reel · Story", reasoning that "Post" alone was ambiguous. Q pushed back — "use their name, not what I labeled it." Both sources agreed the compound was simply wrong: `bundlePublisher.js` sends a literal `type: 'POST'|'REEL'|'STORY'` with **no carousel type at all**, and Meta's `media_product_type` is `FEED`/`REELS`/`STORY` — a carousel is a `media_type` *detail of* a Feed post, not a sibling category. "Post/Carousel" invented a fourth thing and implied carousel was co-equal with Reel.) **Rule: before putting a platform's entity name in any user-facing string or mockup, confirm it twice — grep OUR outbound payload for the literal enum we send (`api/_lib/social/bundlePublisher.js` for social; the provider client for anything else), and check the provider's own docs — and prefer their term verbatim even when it feels less descriptive. Where our internal enum already mirrors it (`content_plan_atoms.format` = post/reel/story), that enum is the answer.** The corollary is cheap and worth it: a label that can't be traced to a real enum value or a doc line is a label you invented.
