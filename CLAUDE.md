...
**PostgREST `in.()` filter syntax: bare values only — double-quoted strings silently match zero rows.** When building an `in.()` filter for a list of UUIDs or strings, use `(${ids.join(',')})` — no quotes around the values. PostgREST accepts bare values or single-quoted strings inside `in.()`; double-quoted strings (`"uuid1","uuid2"`) are invalid syntax that returns zero rows with no error, no 4xx, no log line. This burned `api/_routes/cron/auto-detect-clips.js` (2026-06-18, PR #1391): the cron ran on schedule, logged success, and processed nothing for an unknown period. Correct pattern: `&workspace_id=in.(${wsIds.join(',')})`. Wrong pattern: `&workspace_id=in.(${wsIds.map(id => `"${id}"`).join(',')})`. Also validate every query param that lands in a PostgREST filter with `UUID_RE` before interpolating — a missing check on `staffId` (same audit) left a filter-injection gap while `interviewId` right next to it was correctly validated.
