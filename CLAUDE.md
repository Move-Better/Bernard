...
## caption-fidelity fixture — 30-day calendar expiry kills CI on any open PR

`scripts/verify-caption-fidelity.mjs` fails if `.claude/voice-fidelity-captions-fixture.json`'s `meta.generatedAt` is >30 days old. This is a time-based gate — it fires regardless of code changes. When CI fails with `fixture is N.Nd old (max 30d). Refresh it.`: run `scripts/voice-fidelity-captions.mjs --fixture-out=.claude/voice-fidelity-captions-fixture.json` with real AI_GATEWAY + SUPABASE creds (from `.env.bernard.1pw`); if there are no new scorable packages, bumping `meta.generatedAt` in the JSON to today is acceptable (scores remain valid). Commit the updated fixture and push to unblock CI.

## `isOrgAdmin` vs `role === 'admin'` — owner-only UI gates must use the former

`useUserRole()` computes `role` with an **internal-plan bypass**: `internalBypass = workspace?.plan === 'internal'` grants `role = 'admin'` to **every** org member on Move Better's own workspaces. So `role === 'admin'` in a visibility gate is effectively open to all staff on internal-plan tenants. For UI that should be visible **only to the actual org owner/admin** (onboarding cards, workspace-configuration surfaces), gate on `isOrgAdmin` (now exported from the hook), which reflects the real Clerk `org:admin` membership unaffected by the bypass.
