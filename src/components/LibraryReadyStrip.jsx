  ...
  return (
    <section className="rounded-xl border border-blue-200 bg-blue-50/40 p-4 sm:p-5">
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-widest text-blue-700">
          <Send className="h-3.5 w-3.5" />
          Ready to distribute
        </div>
        <Badge variant="secondary" className="bg-blue-100 text-blue-700 text-[10px] border-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2.5">
        {pieces.slice(0, 8).map((p) => {
          ...
          return (
            ...
            >
              <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium w-fit ${pm.bg} ${pm.color}`}>
                <Icon className="h-3 w-3" />
                {pm.label}
              </span>
              <div className="text-xs font-semibold text-foreground truncate">{p.clinicianName || 'Unknown clinician'}</div>
              <div className="text-[11px] text-muted-foreground line-clamp-2">{p.topic}</div>
              <VoiceChip provenance={p.provenance} />
              <span className="mt-auto inline-flex items-center justify-end gap-0.5 text-[11px] font-medium text-blue-700 group-hover:text-blue-900 transition-colors">
