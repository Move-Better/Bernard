...
function AutoPublishBadge({ state }) {
  ...
  if (Array.isArray(state.gated_reasons) && state.gated_reasons.length > 0) {
    ...
    return (
      <span
        title={`Held: ${tip}`}
        className="inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-3xs font-bold text-white bg-warning/80 backdrop-blur-sm cursor-help"
      >
        <Zap className="h-2.5 w-2.5" />
        Held
      </span>
          ...
          <>
            <div className="absolute top-2 left-2 flex gap-1 flex-wrap">
              {pkg.broll_status === 'complete' ? (
                <span
                  className="inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-3xs font-bold text-white bg-warning/80 backdrop-blur-sm"
