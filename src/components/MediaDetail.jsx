...
export default function MediaDetail({ asset, onClose, onChange }) {
  ...
  // Track when polling started for this asset, so we can cap it even if the
              ...
              <div className="rounded-lg border-2 border-primary/35 bg-primary/5 p-3 space-y-2">
                ...
                {canEdit ? (
                  <Button size="sm" className="w-full gap-1.5" onClick={() => navigate(`/moments/clip/${asset.id}`)}>
                    <Scissors className="h-3.5 w-3.5" />Open the clip editor
                  </Button>
                ) : (
                  <p className="text-2xs text-muted-foreground">Editors cut this video into clips on Moments.</p>
                )}
