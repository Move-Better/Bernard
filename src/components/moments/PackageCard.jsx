...
import { Loader2, CheckCircle2, XCircle, Sparkles, Play, Pause, Pencil, RefreshCw, AlertTriangle, Clock, ShieldAlert, Mic, Brain, Target, Zap, Ban, Library, Send } from 'lucide-react'
...
function VoiceFidelityBadge({ score, breakdown }) {
  ...
  const color =
    rounded >= 7 ? 'bg-success/80' :
    rounded >= 5.5 ? 'bg-warning/80' :
                     'bg-destructive/80'
...
function AutoPublishBadge({ state }) {
  ...
  if (Array.isArray(state.gated_reasons) && state.gated_reasons.length > 0) {
    ...
    return (
      <span
        title={`Held: ${tip}`}
        className="inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-3xs font-bold text-warning-foreground bg-warning/80 backdrop-blur-sm cursor-help"
      >
        <Zap className="h-2.5 w-2.5" />
        Held
      </span>
...
function SimilarityBadge({ similarity }) {
  ...
  const color =
    pct >= 80 ? 'bg-success/80' :
    pct >= 65 ? 'bg-warning/80' :
                'bg-zinc-500/70'
  return (
...
function CampaignChip({ campaign }) {
  const styleClass = {
    ...
    relationship: 'bg-scheduled/[0.08] text-scheduled border-scheduled/20',
