#!/usr/bin/env node
// One-off: ingests Move Better People + Animals blog posts into the Bernard
...
// Dry-run only needs GitHub access (via gh CLI) — no DB or embedding calls.
// Full ingest requires all three.
const requiredEnvs = DRY_RUN
  ? []
...
// ─── Constants — verified 2026-06-01 against bernard prod ──────────────────
...
const WORKSPACES = {
  people: {
    ...
    // People site has Bernard-published posts mixed in. Only ingest
    // posts the clinician actually wrote (verbatim:true in frontmatter).
  ...
  animals: {
    ...
    blogPath:       'src/content/blog',
    // Animals site has no Bernard-generated posts — ingest everything.
    requireVerbatim: false,
  },
}

// Fallback: Animals posts have author: "Dr. Whitney" but no authorSlug field.
// Map display names → slug for workspaces that don't have authorSlug frontmatter.
const AUTHOR_NAME_TO_SLUG = {
  'Dr. Q':       'michael-quasney',
  'Dr. Whitney': 'whitney-phillips',
  'Dr. Zach':    'zachary-cullen',
}
...
// Only import the indexer for live runs — dry-run never calls it.
const { indexOriginalBlog } = DRY_RUN
  ? { indexOriginalBlog: null }
  : await import('../api/_lib/practiceMemoryRag.js')
...
async function processWorkspace(wsKey) {
  ...
  for (const filename of filenames) {
    ...
    try {
      ...
      // Skip Bernard-generated posts in workspaces where they can exist.
      // verbatim:true marks text the clinician actually wrote; anything without
      // it was generated by the Bernard pipeline and must not train their voice.
      if (ws.requireVerbatim && fields.verbatim !== 'true') {
        console.log(`  SKIP (Bernard-generated)  ${slug}`)
        skipped++
        continue
      }
      ...
      // Prefer explicit authorSlug; fall back to display-name map for repos
      // (e.g. Animals) that don't carry an authorSlug field.
      const authorSlug = fields.authorSlug || AUTHOR_NAME_TO_SLUG[fields.author] || null
      const staffId    = authorSlug ? staffMap[authorSlug] : null

      if (!staffId) {
        console.log(`  SKIP (no staff match)        ${slug}  author="${fields.author}" authorSlug="${fields.authorSlug}"`)
        skipped++
        continue
      }
...
// lane in the Bernard app so the full social-generation pipeline runs.
...
Bernard app (movebetter-animals workspace):
