...
export default async function handler(req, res) {
  ...
  const assets = await assetsRes.json().catch(async (e) => { console.error('[coverage] assets JSON parse failed:', e.message, await assetsRes.text().catch(() => '(body unreadable)')); return null })
  ...
  const packages = await packagesRes.json().catch(async (e) => { console.error('[coverage] packages JSON parse failed:', e.message, await packagesRes.text().catch(() => '(body unreadable)')); return null })
