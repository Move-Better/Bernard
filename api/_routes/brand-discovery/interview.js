// Brand-discovery interview persistence — the voice interview the founder runs
// to derive the workspace's brand brief. Mirrors api/_routes/onboarding/
// interview.js; the synthesize route writes the brief to workspaces.brand_brief.
//
// Pinned to Node runtime — matches api/db/interviews.js and keeps the Edge
// whole-graph bundler from dragging Clerk → node:crypto through.
...
async function dbErr(res, r, msg = 'Database error', status = 500) {
  ...
  console.error(`[brand-discovery/interview] ${msg} — supabase ${r.status}: ${body.slice(0, 500)}`)
...
// Look up the founder's Self-staff row by Clerk user_id; create one if missing.
// Returns { staffId } or { error, status } on failure.
async function findOrCreateFounderStaff(ws, userId, fallbackName) {
  ...
  if (!lookup.ok) {
    ...
    console.error(`[brand-discovery/interview] staff lookup failed: ${lookup.status}: ${body.slice(0, 300)}`)
  ...
  const create = await sb('staff', {
    method: 'POST',
    body: JSON.stringify({ workspace_id: ws.id, user_id: userId, name }),
  })
  if (!create.ok) {
    const body = await create.text().catch(() => '')
    console.error(`[brand-discovery/interview] staff create failed: ${create.status}: ${body.slice(0, 300)}`)
...
export default async function handler(req, res) {
  ...
  // Brand discovery is founder-only — gate on workspace admin role.
  ...
  if (req.method === 'GET') {
    if (id) {
      const r = await sb(
        `brand_discovery_interviews?id=eq.${id}&workspace_id=eq.${ws.id}&select=${SELECT_COLS}`
      )
      if (!r.ok) return dbErr(res, r)
      const data = await r.json()
      return ok(res, data[0] ?? null)
    }
    // No id — the workspace's current non-abandoned brand interview.
    const r = await sb(
      `brand_discovery_interviews?workspace_id=eq.${ws.id}&status=neq.abandoned&select=${SELECT_COLS}&order=created_at.desc&limit=1`
    )
    if (!r.ok) return dbErr(res, r)
    const data = await r.json()
    return ok(res, data[0] ?? null)
  ...
  if (req.method === 'POST') {
    ...
    const r = await sb('brand_discovery_interviews', {
      method: 'POST',
      body: JSON.stringify({
        workspace_id: ws.id,
  ...
  if (req.method === 'PATCH') {
    ...
    // Ownership: only the founder who started it can modify. workspace_id is
    // the multi-tenant fence; owner_id is the per-row check.
    const existing = await sb(
      `brand_discovery_interviews?id=eq.${id}&workspace_id=eq.${ws.id}&select=owner_id,status`
    )
    ...
    const r = await sb(
      `brand_discovery_interviews?id=eq.${id}&workspace_id=eq.${ws.id}`,
