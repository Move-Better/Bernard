...
import { buildAuthorizationUrl, gscRedirectUri, signOAuthState } from '../../../_lib/gscAuth.js'

// POST /api/integrations/gsc/connect
//
// Admin clicks "Connect with Google" in Settings → Integrations. Returns an
// OAuth URL that the browser navigates to (same pattern as drive/connect.js —
// fetch() can't follow a Google consent redirect, so we return { url } and the
// client does window.location.assign).
...
async function handler(req, res) {
  ...
  if (!id) {
    return res.status(503).json({ error: 'gsc-not-configured', message: 'Google OAuth credentials are not set up on this deployment.' })
  }
  ...
  try {
    ...
    console.error('[gsc/connect] state sign failed:', e?.message)
    return res.status(500).json({ error: 'state-sign-failed' })
  }

  const url = buildAuthorizationUrl({ redirectUri: gscRedirectUri(), state })
