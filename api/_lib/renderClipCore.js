...
//
// Shared core for the clip render pipeline. ONE implementation of "parse the
// editor doc → fetch + validate the source asset → render each channel + upload"
// so the SYNCHRONOUS endpoint and the ASYNC export worker never drift.
//
// Used by:
//   • api/editorial/render-clip.js          — sync render (post + ad-export flows)
//   • api/editorial/export-clip.js           — async orchestrator (fail-fast validate)
//   • api/editorial/export-clip-worker.js    — async worker (validate + render + upload)
//
// Split into two steps so the async path can validate up front (return a fast
// 202) and render later on a fresh function budget:
//   resolveClipRender({ ws, body }) → { ok, status?, error?, extra?, asset, params }
//       validates the request, fetches the workspace-scoped source asset, runs
//       the consent gate, resolves channels + clinician name + music, and
//       pre-computes caption words. Pure of the render itself.
//   runClipRender({ ws, asset, params }) → { renders, errors, elapsedMs }
//       the per-channel render + blob upload loop. This is the part that can run
//       for minutes; it MUST live behind a fresh 300s budget for heavy clips.
//
// resolveClipRender + runClipRender together reproduce render-clip.js's original
// behavior byte-for-byte (same status codes, same response fields).
...
import { resolveMusicTrack } from './musicLibrary.js'
...
/**
 * Validate the editor doc, fetch + gate the source asset, and resolve everything
 * the render needs. Returns a discriminated result so callers can map failures
 * to HTTP status codes (sync endpoint) or to a 'failed' render_status (worker).
 *
 * @param {Object} p
 * @param {Object} p.ws    resolved workspace row (must have .id)
 * @param {Object} p.body  the request body (renderBody from the editor)
 * @returns {Promise<
 *   | { ok: true, asset: Object, params: Object }
 *   | { ok: false, status: number, error: string, extra?: Object }
 * >}
 */
export async function resolveClipRender({ ws, body = {} }) {
  const assetId = String(body.assetId || '').trim()
  if (!assetId) return { ok: false, status: 400, error: 'assetId_required' }
  if (!UUID_RE.test(assetId)) return { ok: false, status: 400, error: 'invalid_assetId' }
  ...
  if (startSec !== undefined && !Number.isFinite(startSec)) return { ok: false, status: 400, error: 'startSec must be a number' }
  if (durationSec !== undefined && !Number.isFinite(durationSec)) return { ok: false, status: 400, error: 'durationSec must be a number' }
  ...
  if (!assetRes.ok) return { ok: false, status: 500, error: 'db_error' }
  const assets = await assetRes.json()
  const asset = assets?.[0]
  if (!asset) return { ok: false, status: 404, error: 'asset_not_found' }
  if (asset.archived_at) return { ok: false, status: 404, error: 'asset_archived' }
  ...
  if (asset.consent_status === 'pending' || asset.consent_status === 'revoked') {
    return { ok: false, status: 403, error: 'consent_not_granted' }
  }
  if (!asset.blob_url) return { ok: false, status: 500, error: 'asset_missing_blob_url' }

  const isVideo = asset.kind === 'video'
  const isPhoto = asset.kind === 'photo'
  if (!isVideo && !isPhoto) {
    return { ok: false, status: 415, error: 'unsupported_asset_kind' }
  }
  ...
  for (const c of channels) {
    if (!specMap[c]) {
      return { ok: false, status: 400, error: 'invalid_channel', extra: { channel: c, kind: asset.kind } }
    }
  }
  ...
  // detection (media_assets.transcript_words), slice those words to THIS clip
  // window and hand them to the renderer — it then skips the per-render Whisper
  // pass entirely. An explicit captionWords override (the user fixed a word)
  // wins. Legacy assets (no words) fall back to live transcription in the
  // renderer. Computed once here (identical for every channel).
  const captionWords = captionWordsOverride
    || (isVideo && Array.isArray(asset.transcript_words) && asset.transcript_words.length
      ? sliceWordsToWindow(asset.transcript_words, startSec ?? 0, durationSec ?? 60)
      : null)

  const params = {
    captionText, staffName, channels, isVideo, isPhoto,
    startSec, durationSec, subtitles, overlayPosition, overlaySize,
    captionAccent, captionAnim, captionStyle, cuts, grade, reframe, kenBurns,
    overlays, speed, music, captionWords,
  }

  return { ok: true, asset, params }
}

/**
 * Render each resolved channel and upload it. This is the slow part; callers
 * that can run for minutes (heavy 4K/karaoke clips) MUST wrap it in a fresh
 * function budget. Never throws for a single-channel failure — collects them in
 * `errors` (matching the original render-clip.js contract).
 *
 * @param {Object} p
 * @param {Object} p.ws     resolved workspace row
 * @param {Object} p.asset  the source media_assets row from resolveClipRender
...
export async function runClipRender({ ws, asset, params }) {
  ...
  for (const channel of channels) {
    try {
      ...
      console.error(`[renderClipCore] channel ${channel} failed:`, e?.stack || e?.message || e)
      errors.push({ channel, error: e?.message || 'unknown' })
    }
  }

  return { renders, errors, elapsedMs: Date.now() - renderStartedAt }
}
