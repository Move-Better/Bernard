...
export function getAtomSystemPrompt(workspace, staffName, condition, platform, angle, voiceMode = 'practice', tone = 'smart', voiceNotes = '', brandGuidelines = '', voicePhrases = [], audienceLabel = null, storyTypeLabel = null, campaignContext = '', ownHistoryBlock = '', hasPublishedArticle = false, siblingBlock = '', modelExemplarsBlock = '', isPoint = false) {
...
- cta (final slide): hook + body + cta. CTA = 3 to 5 word action phrase like "Book Your Free Assessment".

Aim for 3 to 5 slides total. The last slide should usually be a "cta" template. Don't repeat the same text across slides. Each slide's blocks should cohere with the slide's template defaults but you can omit/add blocks if it serves the story.
...
${facebookUrlLine}
End with a question that sparks comments. 1 to 2 hashtags max.`,
...
${isPersonal
  ...
  : `Write as ${workspace.display_name} the clinic.`}
${facebookUrlLine}
End with a question that invites comments. 1 to 2 hashtags max.`,
    },
...
No hashtags. Conversational, results-focused.`,
    ...
    tiktok: {
      myth_buster: `Write a 45 to 60 second TikTok / Instagram Reels script for ${workspace.display_name} about ${condition}.
...
[BODY: 30 to 40 seconds]
3 to 4 short punchy points. 1 to 2 sentences each. Plain language. Add [ON SCREEN TEXT: ...] for text overlays.
...
At most 1 to 2 hashtags. Prefer NO link unless the punchline only lands with one. Twitter throttles posts with links.
...
${isPersonal ? `Write in ${firstName}'s first-person voice, like you're posting from your phone, not a brand account.` : `Write as the clinic team but in a personal, conversational register, using first names and "we" rather than third-person clinic-speak.`}
End with an open question or invitation to share experiences. No corporate hashtag stacks. At most 1 to 2 lowercase hashtags if they feel natural.
...
NO link unless it's genuinely the post's purpose. If so, put it on its own line at the end.
...
At most 2 to 3 hashtags, written in CamelCase for screen-reader accessibility (e.g. #PhysicalTherapy not #physicaltherapy).
...
FORMAT: 5 to 8 words maximum. ALL CAPS. This text appears printed over a photo or branded card. It must grab attention in under a second and make someone tap the link sticker.
...
After the overlay text, on a new line output the label LINK_STICKER_TEXT: followed by 2 to 4 words for the sticker label (e.g. "Read more", "Book a visit", "Full story"). Keep it action-oriented.
Output ONLY the overlay text line, then the LINK_STICKER_TEXT line. No other text.`,
  ...
  // Social atoms are always the "short" surface — see pointContentFraming.js
  // for why short formats stay strictly experiential (no room to hedge a
  // mechanism claim responsibly, and a caveat is exactly what gets truncated).
  const pointFramingBlock = pointContentFraming({ isPoint, format: 'short' })
...
- ${NO_EM_DASH_RULE}
- Every hashtag is a single unbroken token: never put a space inside one. Write #MovementIsMedicine, not #MovementIs Medicine or #MovementIsM edicine.
