 ...
 */
export function buildClaimExtractionPrompt(draftBody) {
 ...
 */
export async function extractClaims(draftBody, { generateTextFn } = {}) {
  const body = String(draftBody || '').trim()
  if (!body) return []

  const { instructions, user } = buildClaimExtractionPrompt(body)
